...
def interpolate(grid, k_above, col, thresh):
    ...
    z_above = grid.get_xyz(active_index=a_idx_above)[2] # z coordinate of cell gives height
...
def findOwc(grid, owc_kw, x, y, z, thresh=0.7, owc_offset=0.5):
    ...
    if not col:
        raise ValueError('Could not find cell column for (%.2f, %.2f, %.2f) in grid.' % (x,y,z) )
...
def snap(wp, grid, rest, init, date, owc_offset=0.5, keywords=None, delta=Inf,
    ...
    for idx in range(len(wp)):
        ...
        if new_mode and not snap_mode:
            ...
            print('                      %s %f' % (str(wp.depthType()), wp.windowDepth()))
            snap_mode = new_mode
        ...
        if snap_mode:
            try:
                ...
                owc_exact, new_tvd = findOwc(grid, owc_kw, x, y, z,
        ...
        if snap_mode and idx > 1: # caring about delta at this point
            ...
            d = dist((x,y),(x2,y2))  # the projected distance between prev point and this
            prev_z = prev_wp[2]
        ...
        diff = abs(z-new_tvd)
        if diff > 100:
            doprint('Warning: Observed a vertical adjustment of %d m.  Ignoring.' % int(diff))
            new_tvd = z
            diff = 0
        ...
        if new_cell < 0 and snap_mode:
            doprint('Warning:  Suggested wellpoint inactive.  (%d,%d,%d)\t->\t(%.1f, %.1f, %.1f)' % (i,j,k,x,y,new_tvd))
        permx = Nan
    ...
    recognized_kw = ('LENGTH', 'TVD_DIFF', 'OLD_TVD', 'OWC', 'PERMX', 'SWAT', 'SGAS', 'SOIL')
    for kw in keywords:
